...
- 自带集成图标需要 HA 2026.3 或更新版本，旧版不显示该图标但不影响控制功能。
...
出现“蓝牙 BLE 控制失败：连接中断或音箱未响应”时，HK One 或其他 App 可能正在占用蓝牙 BLE 控制连接。请完全退出这些 App，等待几秒后重试。该错误也可能来自信号、音箱待机或代理异常，并不能单凭报错确认 App 占用；这里指蓝牙 BLE 控制连接，不要求断开用于播放音乐的经典蓝牙音频连接。
